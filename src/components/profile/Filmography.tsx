...
import { FilmCard, FilmCardType } from "@/components/FilmCard";
import {
  AddFilmModal,
  FilmFormData,
} from "@/components/AddFilmModal";
...
/**
 * Sem passo "Editar": "Adicionar filme" (ou o lapis de um filme) abre o
...
export const Filmography: React.FC<FilmographyProps> = ({
  ...
  const removeFilm = async (id: string) => {
    ...
    if (!window.confirm(`Remover "${film.title}" da filmografia?`)) return;
